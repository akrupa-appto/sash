...
mock.module('../src/transcribe.ts', { namedExports: {
  transcribe: async req => {
    ...
    calls.push(text);
    return new Promise(resolve => resolvers.push({ resolve, text }));
  },
} });
mock.module('../extension/settings.js', { namedExports: { readSettings: async () => ({}) } });
...
test('the chunked path transcribes the accumulated buffer, not a lone headerless chunk', async () => {
  ...
  resolvers.length = 0;
  sent.length = 0;
  const startReply = await send({ type: 'offscreen:start', chunkMs: 4000 });
