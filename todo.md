...
- [x] [15] tabs the run opens are mixed in with the user's own. put them in a "sash" chrome tab group in `attachPopup` (needs the `tabGroups` permission). never group the tab the user started from.
...
sash ships no content scripts. its permissions are `debugger` and `tabs`, so every signal lives in the side panel. when the agent works in a tab the user is not looking at, the page says nothing.
...
- [x] give each controlled tab a lease bound to a session and a turn, carrying an instance id so a restarted worker can tell whose lease it is. chatgpt refuses a second claim with `Tab N is already part of browser session M`. sash tracks one tab id and nothing else.
...
- [x] name the reason when a page blocks the run instead of going quiet [9]. chatgpt has a dedicated tool for it with four reasons: `captcha_failed`, `access_denied`, `challenge_loop`, `unexpected_bot_error`, described as "the current tab is blocked by bot detection, a failed CAPTCHA, a hard access denial, or a repeated challenge/login loop". sash currently stops and says nothing useful.
...
- [x] on stop, explicitly decline every pending request instead of dropping them. chatgpt fans out a decline to all six pending request types on interrupt, so nothing is left hanging waiting for an answer that will never come. sash's stop path must not leave a question card alive.
...
- [x] gate browser access per origin, with the wording doing the work: "allow sash to access {origin}?" and a separate, scarier confirm for all sites. sash already has `optional_host_permissions`, so this is mostly asking at the right moment rather than new permission machinery.

### ways in

- [x] add a keyboard shortcut that opens the panel. chatgpt registers `open-codex-side-panel` on `Ctrl+Shift+Period`, `Cmd+Shift+Period` on mac. sash's only entry point is the toolbar icon.
- [x] add a right-click entry that sends the selection or link to sash. chatgpt registers one context menu, "Ask ChatGPT", across page, frame, selection, link, editable, image, video and audio.

### deliberately not copying

- their manifest takes `<all_urls>` plus history, bookmarks, topSites, downloads, sessions, webNavigation and nativeMessaging. sash's narrow host permissions and `optional_host_permissions` are better and stay. their own bundled notes argue for cutting back to `sidePanel`, `storage` and `activeTab`. the one permission worth adding is `tabGroups`.
...
- sash already debounces its tab picker on tab events and already honours `prefers-reduced-motion` for both animations. checked, nothing to do.
