...
- The planner prompt changed, so AGENTS.md requires one real-planner lab run before this ships. That run is still outstanding at this commit — do not claim the `every` path is proven until it lands.
...
- Gemini's dedicated speech model does not take an instruction prompt. For a model matching `/transcribe/i` the request is the audio plus `generationConfig.audioTranscriptionConfig`, and the answer is read from the text parts first and `audioTranscription.words` second. The instruction shape stays for general Gemini models.
