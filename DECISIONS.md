...
- `terminals`: one `qa-chrome` running `bash .cursor/qa-chrome.sh` — headless Google Chrome with a CDP endpoint on `127.0.0.1:9223`. `scripts/verify-extension-browser.mjs` (now `npm run test:browser`) connects to it and drives `extension/browser.js` against a real tab. This is the "browser harness auto-connected to a Chrome instance" the environment provides. Chrome uses `--no-sandbox` (unprivileged container) and `--headless=new` (stays foreground).
- The dev server (`npm start`) is deliberately NOT a terminal: it needs `ANCHOR_API_KEY` (browser sessions) and a real `OPENROUTER_API_KEY` (planner) to do anything, and it is a dev tool, not the shipped product (the Chrome extension is). `/api/health` works with no secrets.
