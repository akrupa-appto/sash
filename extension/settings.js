...
export const PROVIDER_KEYS = { openrouter: 'openrouterKey', openai: 'openaiKey', gemini: 'geminiKey' };
export const defaults = {
  provider: 'openrouter', openrouterKey: '', typesafeKey: '', openaiKey: '', geminiKey: '',
...
export function normalizeSettings(input = {}) {
  const out = { ...defaults };
  for (const key of ['openrouterKey', 'typesafeKey', 'openaiKey', 'geminiKey', 'model', 'jevModel', 'textModel']) {
...
export function validateSettings(s, mode = s.mode) {
  ...
  if (mode === 'careful') {
    ...
    const labels = { openrouter: 'OpenRouter', openai: 'OpenAI', gemini: 'Gemini' };
